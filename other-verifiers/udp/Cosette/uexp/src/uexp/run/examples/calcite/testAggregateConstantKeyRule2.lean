...
denoteSQL ((SELECT  (COUNT(uvariable (right⋅emp_slacker))) FROM1  (table rel_emp) WHERE (equal (uvariable (right⋅emp_deptno)) (constantExpr integer_10)) GROUP BY  (right⋅emp_deptno)) :SQL Γ _) 
= 
denoteSQL ((SELECT  (COUNT(uvariable (right⋅emp_slacker))) FROM1  (table rel_emp) WHERE (equal (uvariable (right⋅emp_deptno)) (constantExpr integer_10)) GROUP BY  (right⋅emp_deptno)) :SQL Γ _)  :=
begin
    intros,
