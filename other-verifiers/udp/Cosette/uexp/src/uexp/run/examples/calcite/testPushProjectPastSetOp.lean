...
denoteSQL ((SELECT1 (right⋅emp_sal) FROM1 (((SELECT * FROM1 (table rel_emp) )) UNION ALL ((SELECT * FROM1 (table rel_emp) ))) ) :SQL Γ _) 
= 
denoteSQL (((SELECT1 (right⋅emp_sal) FROM1 (table rel_emp) )) UNION ALL ((SELECT1 (right⋅emp_sal) FROM1 (table rel_emp) )) :SQL Γ _)  :=
begin
    intros,
