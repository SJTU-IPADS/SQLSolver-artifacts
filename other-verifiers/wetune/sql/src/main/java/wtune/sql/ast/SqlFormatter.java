...
public class SqlFormatter implements SqlVisitor{
  ...
  public boolean enterColumnDef(SqlNode colDef) {
    ...
    if (colDef.isFlag(ColDef_Cons, UNIQUE)) append(" UNIQUE");
    if (colDef.isFlag(ColDef_Cons, PRIMARY)) append(" PRIMARY KEY");
    if (colDef.isFlag(ColDef_Cons, NOT_NULL)) append(" NOT NULL");
  ...
  public boolean enterBinary(SqlNode binary) {
    ...
    final boolean needParen =
        op == MEMBER_OF || ExprKind.Array.isInstance(right) || needParen(binary, right, false);
  ...
  public boolean enterSetOp(SqlNode setOp) {
    ...
    if (option != null && option != SetOpOption.DISTINCT)
