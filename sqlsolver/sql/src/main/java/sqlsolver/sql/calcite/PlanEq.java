...
public class PlanEq {
  ...
  public PlanEq(String sql0, String sql1, String schema, String sqlDialect) {
    SqlSupport.muteParsingError();
    final Schema wetuneSchema = SchemaSupport.parseSchema(sqlDialect, schema);
    final SqlNode sqlAst0 = SqlSupport.parseSql(sqlDialect, sql0);
    if (sqlAst0 != null) {
      sqlPlan0 = PlanSupport.assemblePlan(sqlAst0, wetuneSchema);
    } else {
      sqlPlan0 = null;
    }
    final SqlNode sqlAst1 = SqlSupport.parseSql(sqlDialect, sql1);
    if (sqlAst1 != null) {
