...
public abstract class PlanSupport {
  ...
  public static boolean isLiteralEq(String sql0, String sql1, String schema) {
    try {
      return new sqlsolver.sql.calcite.PlanEq(sql0, sql1, schema, DbSupport.PostgreSQL).isEqTree()
              || new sqlsolver.sql.calcite.PlanEq(sql0, sql1, schema, DbSupport.MySQL).isEqTree();
    } catch (Exception e) {
      return false;
    }
  }
